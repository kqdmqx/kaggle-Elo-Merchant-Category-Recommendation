...
def main():
    ...
    features_df = pd.read_csv("./models/lgb037_fi.csv")
    features = list(features_df[features_df.fi_complex > 5.3660078437543035].feature_name)
    ...
    pd.DataFrame({"feature_name": features}).to_csv("./models/outlier039.csv", index=False)

    id_train = train_data.card_id.values
    X_train = train_data[features].values
    y_train = (train_data.target < -30).astype(int).values
    ...
    lgb_params.num_leaves = 191
    lgb_params.colsample_bytree = 0.7392004943960808
    lgb_params.subsample = 0.4591642034942185
    lgb_params.max_depth = 12
    lgb_params.reg_alpha = 0.07218928543750747
    lgb_params.reg_lambda = 24.665536157491537
    lgb_params.min_split_gain = 0.9377756601644203
    lgb_params.min_child_weight = 28.38369552516757
    lgb_params.min_data_in_leaf = 37
    lgb_params.top_rate = 0.012106263707077458
    lgb_params.other_rate = 0.6236728747238944
