...
def main():
    ...
    features = list(features_df[features_df.fi_complex > 4.25].feature_name)
    ...
    max_value = X_train[X_train < np.Inf].max()
    min_value = X_train[X_train > -np.Inf].min()
    print_info("X_train.max", max_value)
    print_info("X_train.min", min_value)

    X_train[X_train == np.Inf] = max_value
    X_train[X_train == -np.Inf] = min_value

    X_test[X_test == np.Inf] = max_value
    X_test[X_test == -np.Inf] = min_value

    rfr_params = addict.Dict()
    rfr_params.n_estimators = 1000  # 5000
    rfr_params.n_jobs = -1
    rfr_params.random_state = 673
    rfr_params.max_depth = 7
    rfr_params.max_leaf_nodes = 23
    rfr_params.min_samples_leaf = 19

    model = RFRModel(rfr_params)
    print_info("rfr_params", model.model_params)
