...
from data_io import load_train_test_main_merchant, load_train_test_main_merchant_pa
...
def main():
    ...
    train_data, test_data = load_train_test_main_merchant()
    train_data_pa, test_data_pa = load_train_test_main_merchant_pa()
    ...
    train_data_pa = reorder_train_set(train_data_pa)
    print_info("train_data.reorder", train_data.head())
    print_info("train_data_pa.reorder", train_data_pa.head())

    features = ["main_m_lag{}".format(month) for month in range(16)]
    id_train = train_data.card_id.values
    X_train_code = train_data[features].values
    X_train_value = train_data_pa[features].fillna(0).values
    train_smaple_cnt = X_train_value.shape[0]
    X_train_value = np.repeat(X_train_value, 128, axis=1).reshape((train_smaple_cnt, 16, 128))
    X_train = {"code": X_train_code, "value": X_train_value}
    ...
    X_test_code = test_data[features].values
    X_test_value = test_data_pa[features].fillna(0).values
    test_smaple_cnt = X_test_value.shape[0]
    X_test_value = np.repeat(X_test_value, 128, axis=1).reshape((test_smaple_cnt, 16, 128))
    X_test = {"code": X_test_code, "value": X_test_value}
