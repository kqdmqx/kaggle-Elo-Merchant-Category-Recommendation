...
# https://www.kaggle.com/mfjwr1/simple-lightgbm-without-blendings
...
from downcast import save_dataframe32
...
TODAY_DATE = pd.to_datetime("2018-12-01")
...
def train_test(num_rows=None):
    ...
    df['elapsed_time'] = (TODAY_DATE - df['first_active_month']).dt.days
...
def historical_transactions(num_rows=None):
    ...
    # trim
    hist_df['purchase_amount'] = hist_df['purchase_amount'].apply(
        lambda x: min(x, 0.8))

    # Y/N to 1/0
    hist_df['authorized_flag'] = hist_df['authorized_flag'].map(
        {'Y': 1, 'N': 0}).astype(int)
    hist_df['category_1'] = hist_df['category_1'].map(
        {'Y': 1, 'N': 0}).astype(int)
    ...
    hist_df['month_diff'] = ((TODAY_DATE - hist_df['purchase_date']).dt.days) // 30
    ...
    for col in ['category_2', 'category_3']:
        hist_df[col + '_mean'] = hist_df.groupby(
    ...
    # change column name
    hist_df.columns = pd.Index([e[0] + "_" + e[1]
                                for e in hist_df.columns.tolist()])
    ...
    hist_df['hist_purchase_date_uptonow'] = (TODAY_DATE - hist_df['hist_purchase_date_max']).dt.days
    hist_df['hist_purchase_date_uptomin'] = (TODAY_DATE - hist_df['hist_purchase_date_min']).dt.days
...
def new_merchant_transactions(num_rows=None):
    # load csv
    new_merchant_df = pd.read_csv(
        './data/new_merchant_transactions.csv', nrows=num_rows)
    ...
    # trim
    new_merchant_df['purchase_amount'] = new_merchant_df['purchase_amount'].apply(
        lambda x: min(x, 0.8))

    # Y/N to 1/0
    new_merchant_df['authorized_flag'] = new_merchant_df['authorized_flag'].map(
        {'Y': 1, 'N': 0}).astype(int)
    new_merchant_df['category_1'] = new_merchant_df['category_1'].map(
        {'Y': 1, 'N': 0}).astype(int)
    new_merchant_df['category_3'] = new_merchant_df['category_3'].map(
        {'A': 0, 'B': 1, 'C': 2}).astype(int)
    ...
    new_merchant_df['month_diff'] = ((TODAY_DATE - new_merchant_df['purchase_date']).dt.days) // 30
    ...
    # change column name
    new_merchant_df.columns = pd.Index(
        [e[0] + "_" + e[1] for e in new_merchant_df.columns.tolist()])
    ...
    new_merchant_df['new_purchase_date_uptonow'] = (TODAY_DATE - new_merchant_df['new_purchase_date_max']).dt.days
    new_merchant_df['new_purchase_date_uptomin'] = (TODAY_DATE - new_merchant_df['new_purchase_date_min']).dt.days
...
def main(debug=True):
    ...
    df = df[[col for col in df.columns if col not in FEATS_EXCLUDED]].reset_index()
    df.columns = ["newk_" + col if col != "card_id" else col for col in df.columns]

    with timer("save dataframe32"):
        save_dataframe32(features_downcast("newk"), df, keep=["card_id"])
